...
;; (define-syntax-class character
;;   (pattern c:id #:when (and (= (length (syntax->string #'c) 3))
;;                             (equal? (string-prefix? (syntax->string #'c) "'"))
;;                             (equal? (string-suffix? (syntax->string #'c) "'")))))
...
(define-syntax-class variant
  (pattern name:id #:fail-unless
           (string-prefix? (syntax->string #'name) ".")
           "types must be prefixed with ':'"))
...
;; TODO: make stricter rules for the structs
...
(define (parse-match-arm stx)
  (syntax-parse stx
    ...
    ;; FIXME: no chars in the match because
    ;; rackets syntax reader can't read ahead

    ;; [(ch:character body:expr ...+)
    ;;  (match-arm
    ;;    (atom (syntax->datum #'ch))
    ;;    (map parse-expr (syntax->list #'(body ...))))]

    [(n:integer body:expr ...+)
     (match-arm
       (int-lit (syntax->datum #'n))
       (map parse-expr (syntax->list #'(body ...))))]

    [(n:number body:expr ...+)
     (match-arm
       (float-lit (syntax->datum #'n))
       (map parse-expr (syntax->list #'(body ...))))]

    [_ (raise-syntax-error
         'invalid-condition
         "Invalid condition. 
         Maybe _ to mark this arm a wildcard?"
         stx)]))
...
(define (parse-expr stx)
  (syntax-parse stx
    ...
    [(match ~! name:id arms:bracket-list ...)
     (match-expr
       (atom (syntax->datum #'name))
       (map parse-match-arm
            (syntax->list #'(arms ...))))]
